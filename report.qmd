...
**Decisions:**
- Dropped all `_code` columns that were redundant with their `_ans` counterparts to remove unnecessary categorical duplicates.  
...
**Decisions:**
- Set implausible numeric outliers beyond physiologic limits to `NaN`.  
...
**Decisions:**
- Scaled numeric features using **StandardScaler** to standardize mean/variance before penalized regression.  
...
**Decisions:**
- Created derived variables such as `bmi_from_selfreport`, `weight_change_1y_lb`, `ever_smoker`, and `current_smoker` based on raw survey responses.  
...
**Decisions:**
- Preserved natural class proportions in both train/test splits (via `stratify=y`).  
