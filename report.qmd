...
**Why these models:**  
- **Logistic Regression (L2)** was chosen as the primary baseline because it is highly interpretable, each coefficient represents the direction and strength of association between a survey feature and prediabetes likelihood. This transparency makes it suitable for healthcare settings, where practitioners must explain decisions.  
- **Elastic Net** extends logistic regression by allowing sparse feature selection, balancing simplicity (via L1 regularization) with stability (via L2). This helps surface which questionnaire features carry the most predictive power without overfitting noise.  
- **Random Forest** and **HistGradientBoosting** were included to test whether nonlinear interactions such as, how age and sleep jointly influence metabolic risk, could outperform linear methods. These models handle complex relationships that might be missed by logistic regression, but their lack of interpretability makes them secondary candidates for real-world deployment.  
